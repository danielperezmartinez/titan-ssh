...
- `.github/workflows/release.yml` con `on: push: tags: ['v*']`:
  ...
  - **ubuntu (versión antigua por glibc)**: `.deb`, `.rpm`, `tar.gz` de
    ...
    `go test ./...` del agente).
  - Hecho el 2026-09-26: ver **Resultado**. Falta el primer tag real.
...
Falta el primer tag real: `v0.1.0-beta.1` publicado y descargado en Windows,
Linux y el Pixel.
...
- **Nombres publicados**: `titan-ssh-<versión>.apk`, `titan-ssh-<versión>.aab`,
  ...
  interna `X.Y.(Z*100+S)`), `titan-ssh_<X.Y.Z~pre>_amd64.deb`,
  `titan-ssh-<X.Y.Z~pre>-1.x86_64.rpm` y `titan-ssh-<versión>-linux-x64.tar.gz`.
