...
Decisiones: Sigue [[ADR-0003 Modelo de resiliencia por niveles]] y [[ADR-0004 Librería SSH]]. Hereda la ejecución al reconectar (fase ON_RECONNECT y `ReconnectBehavior`) de [[Scripts de inicio por sesión]], reutilizando [[ScriptRunner]].
Bloqueada: []
...
1. **Reconexión de cliente (base):** reconexión SSH por el lado del cliente y
   ...
   muere; al reconectar se abre un shell nuevo (se re-ejecutan los scripts de
   inicio: reutilizar [[ScriptRunner]] con la fase `ON_RECONNECT` y respetar
   `ReconnectBehavior` — re-ejecutar todo / solo restaurar `cd` / no hacer nada.
   Este cableado de re-ejecución es el pendiente que hereda de
